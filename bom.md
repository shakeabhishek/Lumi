...
- **AI HAT+ 2 — ✅ SKIP (vision benchmark PASSED 2026-07-01)** — the **Pi-CPU vision benchmark passed on the actual Pi 5**: MediaPipe HandLandmarker (VIDEO mode) held **16.2 fps under a live `qwen2.5:1.5b` turn** (17.2 fps idle, kiosk running) — above the 10–15 fps target. The Pi 5 CPU sustains continuous vision + a live turn, so **no HAT needed; PCIe lane stays free.** *(The LLM is memory-bandwidth-bound, leaving CPU headroom for XNNPACK vision.)* **Integration note:** MediaPipe (protobuf 4.x) and chromadb (protobuf 7.x) conflict in one venv — run vision in its own process/venv in production.
