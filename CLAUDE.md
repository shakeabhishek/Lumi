...
| Cloud LLM as the OpenClaw operator (V2 unlock for community skills) | We tested OpenClaw 2026.04.20 on the laptop with qwen2.5:1.5b, qwen3:1.7b, and llama-3.1:8b. **None of them can drive OpenClaw's agent loop reliably** — qwen2.5:1.5b silently returns zero payloads; qwen3:1.7b mentions reading memory files instead of answering; llama-3.1:8b talks about "BOOTSTRAP.md" instead of greeting. This isn't a model-size limitation — OpenClaw is designed for Claude/GPT-class models trained extensively on its specific agent prompt format. Hailo can't fit a 30B+ model in 8 GB either. Conclusion: full OpenClaw runtime (where community skills work natively) requires the V2 cloud-LLM path. The same `cloud_llm_*` settings the admin console already collects double as the OpenClaw operator. Architecture: `OpenClawBridge.runtime_mode = "ollama" | "cloud"`. In cloud mode the bridge proxies through OpenClaw's gateway with the cloud API key wired in as OpenClaw's `models.providers.<provider>.apiKey`. V1 hybrid (manifests-as-catalog + direct Ollama tool_calls + Python execution) remains the right design for the local-only fallback even after V2 ships. |
