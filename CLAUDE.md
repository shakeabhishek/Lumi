...
| Cloud LLM as the OpenClaw operator (V2 unlock for community skills) | We tested OpenClaw 2026.04.20 on the laptop with qwen2.5:1.5b, qwen3:1.7b, llama-3.1:8b, and qwen2.5:7b. **None drive OpenClaw's agent loop reliably enough to invoke our skills.** qwen2.5:7b is by far the best result — it follows the agent protocol coherently, calls OpenClaw's bundled `memory_search` tool during bootstrap correctly, and produces intelligible responses — but our `~/.openclaw/workspace/skills/<name>/SKILL.md` manifest-format skills are surfaced as system-prompt **text**, not as OpenAI tool_calls. The 7B model treats them as documentation, articulates a plan ("I will fetch the weather"), then hallucinates a result. To make community skills actually fire, V2 needs one of: (a) cloud LLM as the OpenClaw operator (Claude/GPT trained on this format), or (b) skills repackaged as OpenClaw plugins (JS code, not Markdown manifests) so they auto-register as tool_calls. (a) is the lower-effort path. Architecture: `OpenClawBridge.runtime_mode = "ollama" \| "cloud"`. In cloud mode the bridge proxies through OpenClaw's session API (`npx openclaw agent --agent main --message ...`, not the bare `/v1/chat/completions` shim) with the cloud API key wired in as OpenClaw's `models.providers.<provider>.apiKey`. V1 hybrid (manifests-as-catalog + direct Ollama tool_calls + Python execution) remains the right design for the local-only fallback even after V2 ships. |
