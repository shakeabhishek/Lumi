...
- `gmail_read` — **implemented 2026-08-06** (`skills/google_read.py`). IMAP read-only over an App Password, no OAuth (the OOB flow is gone, loopback redirects can't work headless, and Testing-status refresh tokens die in 7 days — see the module docstring). Two mailboxes: Lumi's own dedicated account and, read-only, the owner's personal Gmail. No send, no delete, no marking-as-read. *(Was listed here as a catalog entry with no implementation from Phase 1 until now.)*
...
| **Dedicated accounts for all external integrations** *(amended 2026-08-06 — primary email is now allowed, read-only)* | Original rule: never use the user's primary email/calendar, so a compromised skill could only reach a limited, purpose-built inbox. **Amended at the user's explicit, twice-stated request:** Lumi keeps her own dedicated Gmail *and* gets **read-only** access to the owner's personal Gmail — a desk companion that can't see your actual mail is answering the wrong question. The original rule's threat model was write access and blast radius, and read-only preserves most of it: no send, no delete, no flag changes. Safeguards that make this defensible rather than just convenient — (a) the two mailboxes are **separate credential pairs**, so `lumi keys list` shows at a glance whether personal access is wired; (b) access is opt-in **by existence** — no credential, no code path, verified by test; (c) read-only is enforced at the protocol level three ways (IMAP `EXAMINE`, `BODY.PEEK` so mail is never marked read, and no STORE/EXPUNGE/COPY/APPEND — see `skills/google_read.py`); (d) message **bodies are never fetched**, only From/Subject/Date; (e) every read is audit-logged with *which* mailbox; (f) both credential pairs are swept by factory reset. **Still not allowed:** any write to either account. |
