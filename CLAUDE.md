...
| ~~Cloud LLM fallback with intelligent routing~~ **(SHIPPED 2026-05-24)** | `RoutedBackend` (in `src/lumi/llm/routed_backend.py`) wraps the local backend with a Gemini cloud fallback. Toggle in `/settings/cloud` → "Enable smart cloud routing". Heuristics: `(a)` reply < 12 chars, `(b)` regex match against boilerplate ("I don't know", "I'm not sure", "can't help", "as an AI language model"), `(c)` explicit per-turn `cloud:` prefix on the user message. Memory-prelude messages are stripped before cloud send (detected by header text). Cloud failure falls back to the local reply. Each escalated turn lands in the audit log as `source=cloud:gemini`. **Still V2:** Anthropic + OpenAI adapters (Gemini is the only adapter today since that's what the user has wired); a smarter classifier than the regex heuristics; per-skill toggle to opt skills out of routing. |
